...
<title>@yield('title','HelpDesk') — HelpDesk AMCHAMDR</title>
...
<nav class="sidebar">
    <div class="sidebar-brand">
        <div class="brand-icon" style="background:#002049">
        ...
        <div>
            ...
            <span class="brand-sub">AMCHAMDR · Soporte TI</span>
        </div>
    </div>
    ...
    <div class="sidebar-nav">
        ...
        <a href="{{ route('tickets.index') }}" class="nav-item {{ request()->routeIs('tickets.index','tickets.show') ? 'active' : '' }}">
            ...
            Mis Tickets
            @php
                $pendientes = \App\Models\Ticket::whereNotIn('estado',['resuelto','cerrado'])
            ...
            @endphp
            @if($pendientes > 0)
            <span style="background:var(--red);color:#fff;border-radius:20px;padding:1px 7px;font-size:10px;font-weight:700;margin-left:auto">{{ $pendientes }}</span>
            @endif
        </a>
        ...
        <a href="{{ route('transparencia.index') }}" class="nav-item {{ request()->routeIs('transparencia.*') ? 'active' : '' }}">
        ...
        <a href="{{ route('tickets.index') }}?estado=nuevo" class="nav-item">
            ...
            @if($nuevos > 0)
            <span style="background:var(--cyan);color:#fff;border-radius:20px;padding:1px 7px;font-size:10px;font-weight:700;margin-left:auto">{{ $nuevos }}</span>
            @endif
        </a>
    ...
    <div class="sidebar-footer">
        <a href="{{ route('perfil.show') }}" style="text-decoration:none;display:flex;align-items:center;gap:10px;padding:8px;border-radius:8px;transition:background .15s" onmouseover="this.style.background='#e8edf5'" onmouseout="this.style.background=''">
            <div class="user-avatar">{{ strtoupper(substr(auth()->user()->nombre,0,1)) }}</div>
            <div>
                <div class="user-name">{{ auth()->user()->nombre }}</div>
                <div class="user-role">{{ auth()->user()->rol_label }}</div>
            </div>
        </a>
        <form action="{{ route('logout') }}" method="POST" style="margin-top:8px">
            @csrf
            <button type="submit" style="background:none;border:none;color:#94a3b8;font-size:12px;cursor:pointer;font-family:inherit;display:flex;align-items:center;gap:6px;padding:4px 8px;transition:color .15s;border-radius:6px;width:100%" onmouseover="this.style.color='#ef4444'" onmouseout="this.style.color='#94a3b8'">
        ...
        <div class="flex items-center gap-3">
            ...
            <div style="position:relative" id="notif-wrap">
                <button onclick="toggleNotif()" style="background:none;border:none;cursor:pointer;width:36px;height:36px;border-radius:8px;display:flex;align-items:center;justify-content:center;position:relative;transition:background .15s" onmouseover="this.style.background='var(--surface-2)'" onmouseout="this.style.background=''">
                    ...
                    <span id="notif-badge" style="display:none;position:absolute;top:4px;right:4px;width:16px;height:16px;background:var(--red);color:#fff;border-radius:50%;font-size:9px;font-weight:700;align-items:center;justify-content:center">0</span>
                </button>
                <div id="notif-panel" style="display:none;position:absolute;right:0;top:calc(100% + 8px);width:340px;background:#fff;border:1px solid var(--border);border-radius:10px;box-shadow:0 8px 24px rgba(0,0,0,.12);z-index:200">
                    <div style="padding:12px 16px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center">
                        <span style="font-size:13px;font-weight:700;color:var(--navy)">Notificaciones</span>
                        <a href="{{ route('notificaciones.index') }}" style="font-size:12px;color:var(--navy);text-decoration:none">Ver todas</a>
                    </div>
            ...
            <form action="{{ route('tickets.index') }}" method="GET" style="position:relative">
                <input type="text" name="buscar" placeholder="🔍 Buscar ticket..." value="{{ request('buscar') }}"
                    style="padding:7px 14px;border:1.5px solid var(--border);border-radius:20px;font-size:13px;font-family:inherit;background:var(--surface-2);color:var(--text);width:200px;outline:none;transition:border-color .15s"
...
function cargarNotificaciones() {
    fetch('{{ route("notificaciones.no-leidas") }}', {headers:{'X-Requested-With':'XMLHttpRequest','Accept':'application/json'}})
    .then(r => r.json()).then(data => {
        const badge = document.getElementById('notif-badge');
        const list  = document.getElementById('notif-list');
        if (data.count > 0) { badge.style.display='flex'; badge.textContent=data.count>9?'9+':data.count; }
            ...
            list.innerHTML = data.items.map(n => `
                <a href="${n.url||'#'}" style="display:flex;gap:10px;padding:12px 16px;border-bottom:1px solid #f1f5f9;text-decoration:none;transition:background .15s" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background=''">
                    ...
                    <div style="flex:1;min-width:0">
                        <div style="font-size:12.5px;font-weight:600;color:var(--navy)">${n.titulo}</div>
                        <div style="font-size:12px;color:var(--text-muted);margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${n.mensaje}</div>
                    </div>
                </a>`).join('');
        }
    }).catch(()=>{});
...
document.addEventListener('click', function(e) {
    ...
    if (wrap && !wrap.contains(e.target)) { document.getElementById('notif-panel').style.display='none'; notifOpen=false; }
});
revisarBadge();
