...
use App\Models\Comentario; use App\Models\ActividadLog;
use App\Mail\TicketCreado;
use App\Models\HistorialTicket;
use App\Models\Notificacion; use App\Mail\TicketActualizado; use App\Mail\TicketResuelto;
...
class TicketController extends Controller {
    ...
    public function store(Request $request) {
        ...
        $prioridad = ($user->puedeGestionar() && $request->filled('prioridad'))
            ? $data['prioridad'] : 'media';

        $categoria   = Categoria::find($data['categoria_id']);
        $solicitante = $user->puedeGestionar() && $request->filled('solicitante_id')
        ...
        } catch (\Exception $e) { /* silencioso si falla el mail */ }

        return redirect()->route('tickets.show', $ticket)->with('success', "Ticket {$ticket->numero} creado. Te enviamos un correo de confirmación.");
    }
    ...
    public function show(Ticket $ticket) {
        ...
        $ticket->load(['categoria','solicitante','tecnico','creadoPor','comentarios.usuario','calificacion','adjuntos.usuario','historial.usuario']);
        $tecnicos = Usuario::where('estado','activo')->whereIn('rol',['tecnico','administrador'])->orderBy('nombre')->get();
        $estados  = Ticket::estados();
        return view('tickets.show', compact('ticket','tecnicos','user','estados'));
    }
    ...
    public function cambiarEstado(Request $request, Ticket $ticket) {
        ...
        // Notificar al usuario si fue resuelto
        if ($request->estado === 'resuelto') {
            try {
                $ticket->load(['solicitante','categoria']);
                Mail::to($ticket->solicitante->correo)->send(new TicketResuelto($ticket));
            } catch (\Exception $e) {}
        }
        ...
        if ($ticket->solicitante_id !== auth()->id()) {
            Notificacion::create([
                ...
                'mensaje'     => "Tu solicitud {$ticket->numero} cambió a: {$ticket->estado_label}",
                'url'         => route('tickets.show', $ticket),
                'referencia'  => $ticket->numero,
            ]);
        }
    ...
    public function asignar(Request $request, Ticket $ticket) {
        ...
        if ($ticket->tecnico_id && $ticket->tecnico_id !== auth()->id()) {
            Notificacion::create([
                'usuario_id'  => $ticket->tecnico_id,
                'tipo'        => 'ticket_asignado',
                'titulo'      => 'Ticket asignado',
                'mensaje'     => "Se te asignó el ticket {$ticket->numero}: {$ticket->titulo}",
                'url'         => route('tickets.show', $ticket),
                'referencia'  => $ticket->numero,
            ]);
        }
    ...
    public function cambiarPrioridad(Request $request, Ticket $ticket) {
        ...
        $ticket->save();
        ActividadLog::registrar('actualizó', 'tickets', "Cambió prioridad {$ticket->numero} a {$ticket->prioridad}", $ticket->numero);
    ...
    public function comentar(Request $request, Ticket $ticket) {
        ...
        // Notificar al solicitante si el técnico publica una actualización pública
        ...
        // Notificar
        $notificarA = auth()->user()->puedeGestionar() ? $ticket->solicitante_id : $ticket->tecnico_id;
        if ($notificarA && $notificarA !== auth()->id() && !$esInterno) {
            Notificacion::create([
                'usuario_id'  => $notificarA,
                'tipo'        => 'comentario',
                'titulo'      => 'Nuevo comentario',
                'mensaje'     => "Nuevo comentario en {$ticket->numero}: {$ticket->titulo}",
                'url'         => route('tickets.show', $ticket),
                'referencia'  => $ticket->numero,
            ]);
        }
